...
function styleClick(el) {
	...
	$('button#saveStyling').unbind('click').bind('click', function(){
		...
		$('#styleEditor .form-group:not(#styleElTemplate) input, #styleEditor .form-group:not(#styleElTemplate) select').each(function(){
...
$(function(){
	...
	for( var key in _Elements.elements ) {
		...
		for( x=0; x<_Elements.elements[key].length; x++ ) {
			...
			newItem = $('<li class="element '+niceKey+'"><iframe src="'+_Elements.elements[key][x].url+'" scrolling="no"></iframe></li>');
